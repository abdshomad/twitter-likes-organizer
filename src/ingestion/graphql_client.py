...
BOOKMARKS_QUERY_ID = "jBae9mR6-YVqG3h744v41Q"
BOOKMARKS_FEATURES = {
    "rweb_tipjar_consumption_enabled": True,
    "responsive_web_graphql_exclude_directive_enabled": True,
    "verified_phone_label_enabled": False,
...
class TwitterGraphQLClient:
    def __init__(self, session_path: Path | str | None = None, auth_token: str = "", ct0: str = ""):
        target_path = None
        if session_path and (isinstance(session_path, Path) or "/" in str(session_path) or ".json" in str(session_path)):
            target_path = Path(session_path)
        elif session_path and not auth_token:
            auth_token = str(session_path)

        if not target_path or not target_path.exists():
            target_path = SESSION_FILE if SESSION_FILE.exists() else (BACKUP_SESSION_FILE if BACKUP_SESSION_FILE.exists() else None)
    ...
    def _extract_tweets_from_instructions(self, instructions: list[dict[str, Any]], source: str = "like") -> tuple[list[dict[str, Any]], str]:
    ...
    def _parse_likes_response(self, data: dict[str, Any]) -> tuple[list[dict[str, Any]], str]:
        ...
        return self._extract_tweets_from_instructions(instructions, source="like")

    def _parse_bookmarks_response(self, data: dict[str, Any]) -> tuple[list[dict[str, Any]], str]:
        instructions = data.get("data", {}).get("bookmark_timeline_v2", {}).get("timeline", {}).get("instructions", [])
        if not instructions:
            instructions = data.get("data", {}).get("bookmark_timeline", {}).get("timeline", {}).get("instructions", [])
        if not instructions:
            instructions = data.get("data", {}).get("user", {}).get("result", {}).get("timeline_v2", {}).get("timeline", {}).get("instructions", [])
        return self._extract_tweets_from_instructions(instructions, source="bookmark")
    ...
    async def fetch_bookmarks_page(self, count: int = 40, cursor: str = "") -> tuple[list[dict[str, Any]], str]:
        ...
        params = {"variables": json.dumps(variables), "features": json.dumps(BOOKMARKS_FEATURES)}
        res = await self.client.get(url, params=params)
        if res.status_code != 200:
            return [], ""

        try:
            return self._parse_bookmarks_response(res.json())
        except Exception:
            return [], ""
    ...
    async def fetch_all_bookmarks_streaming(
        self,
        max_tweets: int = 0,
    ) -> AsyncGenerator[dict[str, Any], None]:
        cursor = ""
        total_fetched = 0
        while True:
            batch, cursor = await self.fetch_bookmarks_page(count=40, cursor=cursor)
            if not batch:
                break
            for tweet in batch:
                yield tweet
